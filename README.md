...
- **ふりがなはビルド時に自動で付けています。** 形態素解析（kuromoji）で読みを取り、漢字を含む語にだけ `<ruby>` を出します。
  ブラウザに解析器は送りません。
- **読み違いは上書きで直します。** 優先度は次の順です。
  1. 各人物 JSON の `rubyOverrides`（その人のページでだけ効く。同じ語の読みが文脈で変わるとき）
  2. `src/data/reading-overrides.ts` の手書き一覧（古語・書名・地名・仏教語など）
  3. `src/data/person-readings.ts`（人物名。各 JSON の `kana` から `python3 scripts/gen_person_readings.py` で生成）
  4. 形態素解析の結果
- **付き方の点検はスクリプトで行います。**

  ```sh
  npx tsx scripts/check-furigana.mjs        # 読みが付かない語と、細切れになった語を洗い出す
  npx tsx scripts/check-furigana.mjs --all  # 全件を出す
  ```

  いまは「読みが付かない語 0 件」です。細切れとして出るものは、短い語が並んでいるだけの正常な例が中心です。
- **ことばの解説**は `src/data/glossary.ts` にまとめてあります。名言の本文に語が現れたページへ自動で出ます。
  1語につき1エントリなので、同じ語を使う名言が増えても書き足す必要はありません。
  説明は小学校高学年が読める言葉で、2文までにします。
- 表示・非表示はヘッダーの「ふりがな」ボタンで切り替わり、選択は `localStorage` に残ります。
